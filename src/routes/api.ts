...
apiRouter.post("/auth/login", async (req, res, next) => {
  try {
    ...
    const resolvedRole = account ? role ?? loginRoleForAccountRole(account.role) : undefined;
    ...
    if (!account || account.passwordHash !== password) {
      await appendAuditLog(prisma, {
        ...
        objectType: "AUTH",
        objectId: identifier,
        newValue: { identifier, reason: "INVALID_CREDENTIALS", role: role ?? "" },
      });
    ...
    if (account.status !== "ACTIVE") {
      await appendAuditLog(prisma, {
        ...
        newValue: { identifier, reason: "ACCOUNT_LOCKED", role: role ?? "" },
      });
      res.status(403).json({ message: "Account is locked" });
      return;
    }

    if (!resolvedRole || !accountRoleMatchesLoginRole(account.role, resolvedRole)) {
      await appendAuditLog(prisma, {
        actorId: account.accountId,
        ipAddress: req.ip,
        userAgent: req.get("user-agent") ?? null,
        ...
        objectType: "AUTH",
        objectId: account.accountId,
        newValue: { identifier, reason: "ROLE_FORBIDDEN", role: role ?? "" },
      });
      res.status(403).json({ message: "Forbidden for requested role" });
      return;
    }
    ...
    await appendAuditLog(prisma, {
      ...
      objectType: "AUTH",
      objectId: account.accountId,
      newValue: { identifier, role: resolvedRole },
    });
