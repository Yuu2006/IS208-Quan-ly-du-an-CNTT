...
export function TimelineItem({ title, completedAt, done, last, description = 'Bản ghi chuỗi cung ứng đã xác minh' }: { title: string; completedAt: string; done: boolean; last: boolean; description?: string }) {
  return <div className="flex gap-3"><div className="flex flex-col items-center"><div className={`grid h-8 w-8 place-items-center rounded-full ${done ? 'bg-leaf text-white' : 'bg-slate-100 text-muted'}`}>{done ? <Check size={16} /> : <CircleDot size={16} />}</div>{!last && <div className="h-12 w-px bg-line" />}</div><div className="pt-1"><p className="font-bold text-ink">{title}</p><p className="text-xs text-muted">{description}</p><p className={`mt-1 inline-flex items-center gap-1 text-xs font-bold ${done ? 'text-leaf' : 'text-muted'}`}><CalendarCheck size={13} /> {completedAt}</p></div></div>;
}
